...
read_vars = ['ADIFR', 'BDIFR', 'ADIFRTEMP', 'BDIFRTEMP', # adifr, bdifr
             'QCF', 'QCR', 'QC_A', 'QC_A2', # raw, dynamic pressures (q)
             'GGLAT','GGLON','GGNSAT','GGQUAL','GGSPD','GGTRK', # GPS variables
             'PSF', 'PSXC', # static pressures
...
class aoa_fit:
    ...
    def __init__(self, df: pd.DataFrame, mask: np.array, flight: str, leg: int = None, order=5, sampling_freq=2*math.pi/1, cutoff_freq=2*math.pi/30.):
        ...
        self.flight = flight
        self.leg = leg
        self.prev_coefs_three = [4.945466, 16.363147, 2.339622] # started with ACCLIP-high coefs
        self.varnamemap = {'datetime': 'dt', 'QCF': 'q', 'PSF': 'ps', 'ADIFR': 'adifr', 'BDIFR': 'bdifr', 'SSLIP': 'sslip',
                           ...
                           'GGALT': 'alt', 'ATX': 'tc', 'PSXC': 'p', 'UIC': 'u', 'VIC': 'v', 'WIC': 'w', 'GGVEW': 'up', 'GGVNS': 'vp', 'GGTRK': 'trk', 'ROLL': 'roll'}
    ...
    def calc_coefs(self):
        ...
        self.coefs_two, pcov_two, odict_two, msg_two, ierr_two = curve_fit(simple_fit_func, ratio[finite_mask], self.aoa_ref[finite_mask], 
                                                                           p0=self.prev_coefs_three[0:2], full_output=True, method='trf')
...
def plot_maneuv_for_aoa(aoa_obj: aoa_fit):
    ...
    colors = itertools.cycle(Category10[8])
    proll = figure(width=width, height=height, x_range=p1.x_range)
    proll.add_layout(Title(text="Roll [deg]", align="center"), "left")
    proll.line(aoa_obj.dt, aoa_obj.roll, color=next(colors))
    proll.add_tools(ht)
    format_ticks(proll)
    ...
    p = gridplot([[p8], [p2], [p1], [proll], [p3], [p7], [p9], [p5]])
    show(p)
...
def plot_aoa_scatter_for_cal(aoa_obj: aoa_fit, title='', aoa_range=(0,6.5), aos_range=(-6.5,6.5)):
    ...
    p1.add_layout(Title(text="AoA Ref [deg]", align="center"), "left")
    p1.add_layout(Title(text="ADIFR/Q", align="center"), "below")
    p1.dot(aoa_obj.adifr/aoa_obj.q, aoa_obj.aoa_ref, color='black', size=10)
    p1.line(aoa_obj.adifr/aoa_obj.q, aoa_obj.akrd_two, color='red', legend_label='1-predictor fit', width=2)
    p1.line(aoa_obj.adifr/aoa_obj.q, aoa_obj.akrd, color=next(colors), legend_label='ACCLIP fit')
    p1.line(aoa_obj.adifr/aoa_obj.q, aoa_obj.akrd_three, color=next(colors), legend_label='CGWAVES fit')
    p1.legend.location = 'top_left'
...
def plot_hists_ascent_descent(ascent, descent, nbins=50):
    ...
    up_mean = np.mean(w_up)
    up_std  = np.std(w_up)
    up_skew = skew(w_up)
    dn_mean = np.mean(w_dn)
    dn_std  = np.std(w_dn)
    dn_skew = skew(w_dn)
    ...
    up_bins = np.linspace(up_min, up_max, nbins)
    up_hist, up_edges = np.histogram(w_up, density=True, bins=up_bins)

    dn_bins = np.linspace(dn_min, dn_max, nbins)
    dn_hist, dn_edges = np.histogram(w_dn, density=True, bins=dn_bins)

    height = 350
    width = 600

    title_str = f"Ascent Mean: {up_mean:4.2f}, Std: {up_std:4.2f}, Descent Mean: {dn_mean:4.2f}, Std: {dn_std:4.2f}"
    p = figure(width=width, height=height, title=title_str)
    p.add_layout(Title(text="WIC [m/s]", align="center"), "below")
    p.add_layout(Title(text="Prob.", align="center"), "left")
    p.quad(top=up_hist, bottom=0, left=up_edges[:-1], right=up_edges[1:], fill_color='white', line_color='black', legend_label='Ascent', fill_alpha=0, line_width=2)
    p.quad(top=dn_hist, bottom=0, left=dn_edges[:-1], right=dn_edges[1:], fill_color='white', line_color='red', legend_label='Descent', fill_alpha=0, line_width=1)
